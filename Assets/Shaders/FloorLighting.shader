...
Shader "Custom/FloorLighting" {
    Properties {
        ...
        [MaterialToggle] _StripeDir ("StripeDir", Float ) = 0
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float _StripeDir_var = lerp( i.posWorld.r, i.posWorld.b, _StripeDir );
                float4 node_266 = _Time;
                float3 emissive = ((_Emission_var.rgb*_EmissionColor.rgb)*saturate((sin(((_StripeDir_var+((_StripeDir_var+((node_266.r*_TimeMult)*(-1.0)))+i.uv0.g.r).r)*_LineScale*6.28318530718))*2.0+-1.0)));
/// Final Color:
                outDiffuse = half4( diffuseColor, 1 );
                outSpecSmoothness = half4( specularColor, gloss );
                outNormal = half4( normalDirection * 0.5 + 0.5, 1 );
                outEmission = half4( ((_Emission_var.rgb*_EmissionColor.rgb)*saturate((sin(((_StripeDir_var+((_StripeDir_var+((node_266.r*_TimeMult)*(-1.0)))+i.uv0.g.r).r)*_LineScale*6.28318530718))*2.0+-1.0))), 1 );
                #ifndef UNITY_HDR_ON
