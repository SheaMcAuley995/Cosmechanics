...
////// Emissive:
                ...
                float3 emissive = lerp(_BLACK.rgb,_Emissive.rgb,saturate((sin(((_XZ_var+((_XZ_var+((node_266.r*_TimeMult)*_FR))+i.uv0.g.r).r)*_LineScale*6.28318530718))*0.5+0.5)));
                float3 finalColor = emissive;
                outDiffuse = half4( 0, 0, 0, 1 );
                outSpecSmoothness = half4(0,0,0,0);
                outNormal = half4( normalDirection * 0.5 + 0.5, 1 );
                outEmission = half4( lerp(_BLACK.rgb,_Emissive.rgb,saturate((sin(((_XZ_var+((_XZ_var+((node_266.r*_TimeMult)*_FR))+i.uv0.g.r).r)*_LineScale*6.28318530718))*0.5+0.5))), 1 );
                #ifndef UNITY_HDR_ON
